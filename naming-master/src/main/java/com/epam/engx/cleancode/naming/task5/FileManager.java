...
import com.epam.engx.cleancode.naming.task5.predicates.FileExtensionPrediction;
...
public final class FileManager {
    ...
    private static final String[] DOCUMENT_TYPE = {"pdf", "doc"};
    ...
    public List<String> listAllImages() {
        return files(basePath, IMAGE_TYPE);
    }

    public List<String> listAllDocumentFiles() {
        return files(basePath, DOCUMENT_TYPE);
    }
    ...
    private boolean isInvalidImage(String fileName) {
        FileExtensionPrediction imageExtensionsPredicate = new FileExtensionPrediction(IMAGE_TYPE);
        return !imageExtensionsPredicate.test(fileName);
    }

    private boolean isInvalidDocument(String fileName) {
        FileExtensionPrediction documentExtensionsPredicate = new FileExtensionPrediction(DOCUMENT_TYPE);
        return toggleBoolean(documentExtensionsPredicate.test(fileName));
    }
    private boolean toggleBoolean(boolean boolValue){
    	return !boolValue;
    	
    }
    private List<String> files(String directoryPath, String[] allowedExtensions) {
        final FileExtensionPrediction pred = new FileExtensionPrediction(allowedExtensions);
        return Arrays.asList(directory(directoryPath).list(getFilenameFilterByPredicate(pred)));
    }

    private FilenameFilter getFilenameFilterByPredicate(final FileExtensionPrediction pred) {
